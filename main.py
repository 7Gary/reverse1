...
def train(_class_):
    ...
    # 使用 load_data 加载数据集
    train_dataloader, test_dataloader = load_data(dataset_name=_class_, batch_size=batch_size, input_size=image_size)

    ckp_path = './checkpoints/' + 'wres50_' + _class_ + '.pth'
    os.makedirs('./checkpoints', exist_ok=True)  # 确保目录存在
    ...
    for epoch in range(epochs):
        ...
        decoder.train()
        loss_list = []
        for img, gt, label, img_type in train_dataloader:
            img = img.to(device)
            inputs = encoder(img)
